...
const writeChangelog = releaseInfo => {
    ...
    // Leave CHANGELOG.tmp in so Concourse can use it.
 ...
 * @param {string} [branchName] The name of the branch that we are releasing from.
 * @returns {Object} The information about the release.
 */
const release = (prereleaseId, branchName) => {
    const pkg = {
        ...getPackageInfo(),
        isPublic: IS_PUBLIC
    };
    ...
    ShellOps.exec(`git push origin HEAD:${branchName} --tags --verbose`);
    ...
    // If we are prereleasing from a non-master branch, then there is no need to commit the extra files back to a branch
    // They will still be in the prerelease npm package.
    // Any extra files will eventually get committed when we run the real release from the master branch.
    if (!prereleaseId) {
        ShellOps.exec('git add -A');
        ShellOps.exec('git diff --quiet && git diff --staged --quiet || git commit -am "publish: Post-publish add of untracked files"');
        ShellOps.exec('git push origin HEAD:master');
    }
